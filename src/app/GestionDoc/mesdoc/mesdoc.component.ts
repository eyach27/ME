...
@Component({
  ...
  styleUrls: ['./mesdoc.component.scss']
})
...
users:any;
  ...
  reloadData() {
    ...
    this.documents.subscribe((docs) => {
      const loggedInUser = this.tokenStorageService.getUser().username;
      const filteredDocs = docs.filter(doc => doc.owner === loggedInUser); // Filter docs by owner
      ...
      this.documents = filteredDocs; // Update documents with filtered docs
    });
  }
