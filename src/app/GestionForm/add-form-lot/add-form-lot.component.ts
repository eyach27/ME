...
@Component({
  selector: 'add-form-lot-formation',
  templateUrl: './add-form-lot.component.html',
  styleUrls: ['./add-form-lot.component.scss'],
  
})
export class AddFormLotComponent implements OnInit {
...
description: string = '';
  ...
  nameLot: any;
  constructor(private gestionDocService: GestionDocService,private themeService: ThemeService,private gestionPlanService : gestionPlanService,private gestionFormService : gestionFormService, private gestionLotService : gestionLotService,
 private userServiceGestService: UserServiceGestService,private route: ActivatedRoute, private departementService:DepartmentService, private tokenStorageService : TokenStorageService,private router: Router,private modalService: NgbModal,private datePipe: DatePipe) { 
  ...
  ngOnInit(): void {
    ...
    // Fetch the lot data based on the id
    this.gestionLotService.getLotById(this.id).subscribe((data: Lot) => {
      this.AddToLot = data;
      this.nameLot = this.AddToLot.name;
    });
...
this.gestionLotService.addNewPlanning(formDataP).subscribe(
  (result) => {

        this.valid = true;
        this.messagevalid = ("Formation planifiée ajoutée avec succés");
        setTimeout(() => {
          this.router.navigate(['/edit-lot/',this.id]) ;
        }, 2000);
  },
  (error) => {
    this.failed = true;
