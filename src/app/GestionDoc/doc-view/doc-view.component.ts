...
users:any;
  ...
  downloadmultidoc(){
   ...
   this.downloadDoc(doc.id,doc.name);
    ...
    deleteadmultidoc(content, id) {
      this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
        ...
        if (result === 'yes') {
          ...
          for (let doc of this.selectedDocs) {
            if (this.deleta(doc) || this.showAdminBoard) {
              this.deleteDoc(doc.id);
            } else {
              this.failed = true;
              this.errorMessage = ("Non autorisé à supprimer les documents sélectionnés");
              setTimeout(() => {
                this.failed = false;
              }, 1500);
            }
    ...
    deleteDoc(id: number) {
      this.gestionDocService.deleteDoc(id).subscribe(
        data => {
          this.reloadData();
          this.test = true;
          this.message = 'Document supprimé avec succée';
          setTimeout(() => {
            this.test = false;
          }, 1500);
        },
        error => {
          this.failed = true;
          this.errorMessage = 'La suppression du document est échouée';
          setTimeout(() => {
            this.failed = false;
          }, 1500);
        }
    ...
    deleta(doc: any) {
      if (this.tokenStorageService.getUser().username == doc.owner) {
        return true;
      } else {
        return false;
      }
  ...
  this.showicon=false;
    ...
    opene(contente,id) {  
      ...
      .subscribe(data => {
        ...
        for(var i=0;i<this.doc.departments.length;i++){
          this.selectedDepartments.includes(this.doc.departments[i])
...
if (this.formSearch.filtre === "type") {
 ...
 this.documents = this.gestionDocService.getDocByType(this.formSearch.rech.toLowerCase());
  
}
  
if (this.formSearch.filtre === "dep") {
